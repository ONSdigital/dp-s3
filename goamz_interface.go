...
// AmzClient  - interface representing an s3 client wrapper for goamz s3 package
type AmzClient interface {
	GetBucketReader(bucketName string, path string) (io.ReadCloser, error)
}

// AmzClientImpl - Implementation of the AmzClient interface wrapping the real S3 calls to nested structs (e.g Bucket)
type AmzClientImpl struct {
	s3 *goamzs3.S3
}

// GetBucketReader - get io.ReadCloser from bucket name and path, using the Bucket struct in goamz
